...
from . import api_query, yaml_dump
...
- name: Creating 'git' subdomain
  community.alwaysdata.dnsrecord:
    domain: example.com
...
def state_present(module, token, domain, filtered_records):
    ...
    if not filtered_records:
        ...
        if not module.check_mode:
            create_dnsrecord(module, token, **params.to_api_params())

        result["changed"] = True
        result["diff"] = {
            "before": "",
            "after": yaml_dump(params.to_api_params()),
        }
    ...
    for record in filtered_records:
        ...
        before.append(old_params.to_api_params())
        ...
        if not set(old_params.to_api_params().items()) - set(updated_params.items()):
            after.append(old_params.to_api_params())
            continue

        result["changed"] = True
        after.append(params.to_api_params())
        if not module.check_mode:
            update_dnsrecord(module, token, record["id"], **params.to_api_params())

        after.append(updated_params)

    result["diff"] = {
        "before": yaml_dump(before),
        "after": yaml_dump(after),
    }
...
def dnsrecord():
    ...
    def filter_record(record):
        ...
        if module.params.get("type"):
            module.fail_json(msg=f"prereq={prereq}")

        if module.params.get("regex"):
            return prereq and re.compile(module.params["regex"]).match(record["value"])

        return prereq and (
            module.params.get("value") is None or record["value"] == module.params["value"]
        )
